function LL=LogLikeLDS(A,W,S,Inp_,mu0_,B,G,h,X_,Z_)
%
...
for i=1:ntr
    H=repmat(h,1,T(i));
    P=Z{i}(:,1)-mu0{i}-Inp{i}(:,1); LL0=P'*S^-1*P;  % from initial state
    P=Z{i}(:,2:T(i))-A*Z{i}(:,1:T(i)-1)-W*(Z{i}(:,1:T(i)-1)-H(:,1:T(i)-1))-Inp{i}(:,2:T(i));
    LL1=trace(P'*S^-1*P);   % from process (latent state) model
    P=X{i}-B*(Z{i}-H); LL2=trace(P'*G^-1*P);   % from observation model
