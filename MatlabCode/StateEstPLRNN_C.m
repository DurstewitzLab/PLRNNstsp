function [z,U,d,Err]=StateEstPLRNN_C(A,W,C,S,Inp_,mu0_,B,G,h,X_,z0,d0,tol,eps,flipAll)
%
...
for i=1:ntr   % acknowledge temporal breaks between trials
    ...
    I=C*Inp{i};
    vka=S^-1*I; vka(:,1)=vka(:,1)+S^-1*mu0{i};
    vkb=A'*S^-1*I(:,2:T);
...
hh=repmat(h,Tsum,1);
n=1; idx=0; k=[];
if nargin>10 && ~isempty(z0), z=z0(1:end)'; else z=hh+randn(m*Tsum,1); end;
if nargin>11 && ~isempty(d0), d=d0; else d=zeros(1,m*Tsum); d(z>hh)=1; end;
Err=1e16;
